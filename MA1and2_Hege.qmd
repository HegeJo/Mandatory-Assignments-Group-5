...
```{r}
22_MSB104_Group_5_AS_Sugestions
#| echo: false
#| output: true
...
#| echo: false
#| output: true
#| label: fig-2010_log_gdp_gini
#| fig-cap: "Log(GDP pc.) vs wGini simple linear regration"
...
```{r echo=FALSE}
 ...
 relationship_total <- data2010_nuts2 %>% 
  ggplot() +
  geom_point(aes(x = log_GDP_pc, y = gini_n2)) +
  geom_abline(intercept = -0.351, 
              slope = 0.044, 
              colour = "green", 
              size = 1) + 
  theme_minimal() +
  ggtitle("Total: Relationship between regional development and regional  inequality") + 
...
```{r echo=FALSE}
relationship_UK <- UK2010_nuts2 %>% 
 ggplot() +
 geom_point(aes(x = log_GDP_pc, y = gini_n2)) +
 geom_abline(intercept = -1.997 , 
             slope = 0.206, 
             colour = "purple",  
             size = 1) + 
 theme_minimal() +
    ggtitle("UK: Relationship between regional development and regional  inequality") + 
  xlab("log(GDP per capita)") + 
  ylab("Gini-coefficient")
...
```{r echo=FALSE}
resi_vs_pred_all <- lm.1

resi_vs_pred_all %>% 
  ggplot(aes(x = fitted(resi_vs_pred_all), y = resid(resi_vs_pred_all))) +
  geom_point(colour = "darkslategrey", size = 2) +
  xlab("Predicted values") + ylab("Residuals") +
  ggtitle("All countries: Residuals vs. preditcted values") +
  theme_minimal()

```

```{r echo=FALSE}
resi_vs_pred_UK <- lm.2

resi_vs_pred_UK %>% 
  ggplot(aes(x = fitted(resi_vs_pred_UK), y = resid(resi_vs_pred_UK))) +
  geom_point(colour = "darkslategrey", size = 2) +
  xlab("Predicted values") + ylab("Residuals") +
  ggtitle("UK: Residuals vs. preditcted values") +
  theme_minimal()
```
...
Just the list of assumtions, nothing written yet.
main

1.  **Linearity:** The population relationship is linear in parameters with an additive error term.
    <br>

2.  SLR: **Sample Variation:** There is variation in $X$.
    vs. <br>

    MLR: **No perfect collinearity:** No $X$ variable is a perfect linear combination of the others.
...
**Pleas talk about this table regarding inferences and model fit. You do not need to do separate t test as the standard divination and p values are documented in the table. e.g. three stars mean that a effect is significant at a 1% level etc.  
