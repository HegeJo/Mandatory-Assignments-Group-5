...
# creating new dataframe for all nuts 2
data2010_nuts2 <- GDPperCap_all_nuts3 %>%
  filter(Year == 2010) %>% distinct(id_nuts2, .keep_all = TRUE) %>% 
  select(id_nuts2, GDP, Population, GDP_per_capita, gini_n2)
...
```{r}
CzechR2010_nuts2 <- data2010_nuts2 %>% 
  filter(grepl("CZ..", id_nuts2))

Finland2010_nuts2 <- data2010_nuts2 %>% 
  filter(grepl("FI..", id_nuts2))

Turkey2010_nuts2 <- data2010_nuts2 %>% 
  filter(grepl("TR..", id_nuts2))
...
```{r}
# estimates for the different countries

lm_CzechR <- lm(gini_n2~log_GDP_pc, CzechR2010_nuts2)
lm_Finland <- lm(gini_n2~log_GDP_pc, Finland2010_nuts2)
lm_Turkey <- lm(gini_n2~log_GDP_pc, Turkey2010_nuts2)
lm_UK <- lm(gini_n2~log_GDP_pc, UK2010_nuts2)
```

```{r}

list("CzechRepublic" = lm_CzechR)

```

```{r}
list("Finland" = lm_Finland) 
```

```{r}
list("Turkey" = lm_Turkey)
```

```{r}
list("United Kingdom" = lm_UK)
```
...
```{r}
huxtable(
  list("Czech" = lm_CzechR, "Finland" = lm_Finland, "Turkey" = lm_Turkey, "UK" = lm_UK),
  statistics = c(N = "n-obs", R2 = "r.squared"),
...
```{r}
huxreg(
 list("All countries" = lm_allcountries, "Czech" = lm_CzechR, "Finland" = lm_Finland, "Turkey" = lm_Turkey, "UK" = lm_UK),
...
```{r}
# Save coefficients
b1_Czech <- lm_CzechR$coefficients[1]
b2_Czech <- lm_CzechR$coefficients[2]

CzechR2010_nuts2 %>% 
  ggplot() +
  geom_point(aes(x = log_GDP_pc, y = gini_n2)) +
  geom_abline(intercept = b1_Czech, slope = b2_Czech, colour = "green", size = 1) +
 theme_minimal()
```

```{r}
# Save coefficients
b1_Finland <- lm_Finland$coefficients[1]
b2_Finland <- lm_Finland$coefficients[2]

Finland2010_nuts2 %>% 
  ggplot() +
  geom_point(aes(x = log_GDP_pc, y = gini_n2)) +
  geom_abline(intercept = b1_Finland, slope = b2_Finland, colour = "green", size = 1) +
 theme_minimal()
```

```{r}
# Save coefficients
b1_Turkey <- lm_Turkey$coefficients[1]
b2_Turkey <- lm_Turkey$coefficients[2]

Turkey2010_nuts2 %>% 
  ggplot() +
  geom_point(aes(x = log_GDP_pc, y = gini_n2)) +
  geom_abline(intercept = b1_Turkey, slope = b2_Turkey, colour = "green", size = 1) +
 theme_minimal()
```

```{r}
# Save coefficients
b1_UK <- lm_UK$coefficients[1]
b2_UK <- lm_UK$coefficients[2]

UK2010_nuts2 %>% 
  ggplot() +
  geom_point(aes(x = log_GDP_pc, y = gini_n2)) +
  geom_abline(intercept = b1_UK, slope = b2_UK, colour = "green", size = 1) +
 theme_minimal()
```
...
# Renaming geo column
      transport2010_nuts2 <- rename(transport2010_nuts2,   "Region_code" = "GEO (Codes)")
```
...
# Renaming geo column
      demographics2010_nuts2 <- rename(demographics2010_nuts2,   "Region_code" = "GEO (Codes)")
```
...
education2010_nuts2 <- education2010_nuts2 %>% 
   ...
   pivot_longer(
    cols = c('2010'),
    names_to = "Year",
    values_to = "share_of_student/pupils"
  )
 
# Renaming geo column
      educ_regind_custom_3797513_spreadsheet <- rename(education2010_nuts2, "Region_code" = "GEO (Codes)")
```
