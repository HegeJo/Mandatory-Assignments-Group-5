...
[^2]: Source: <https://www.gov.uk/search/research-and-statistics>
...
[^4]: Source: https://www.ons.gov.uk/economy/grossdomesticproductgdp/datasets/regionalgrossdomesticproductallnutslevelregions
...
### Effect of regional development on regional inequality for 2010

Economical development is the logarithm of GDP per capita, according to [@lessmannRegionalInequalityConvergence2017].
...
$R_2$ is the share of the explained variation compared to the total variation and is between 0 and 1[@wooldridgeIntroductoryEconometricsModern2020, p.35].
...
The data used in this assignment is collected from Eurostat[^9].
The data used is from 2010 and is for NUTS2 regions.

[^9]: Source: https://ec.europa.eu/eurostat/web/regions/data/database

#### Transport infrastructure

For this category I have selected the variable total railway lines (Railway_km), which is measured in kilometers.
The data is imported from the dataset TGS00113[^10], which is computed from the dataset TRAN_R\_NET[^11]
.

[^10]: Source: <https://ec.europa.eu/eurostat/databrowser/view/tgs00113/default/table?lang=en>

[^11]: Source: <https://ec.europa.eu/eurostat/databrowser/view/TRAN_R_NET/default/table?lang=en>
...
For this category I have selected the variable population density(Population_density), which is measured in number of population per square kilometer.
The data is imported from the dataset TGS00024[^12], which is computed from the dataset DEMO_R\_D3DENS [^13]
.

[^12]: Source: <https://ec.europa.eu/eurostat/databrowser/view/tgs00024/default/table?lang=en>

[^13]: Source: <https://ec.europa.eu/eurostat/databrowser/view/DEMO_R_D3DENS/default/table?lang=en>[(europa.eu)](https://ec.europa.eu/eurostat/databrowser/view/DEMO_R_D3DENS/default/table?lang=en)
...
For this category I have selected the variable: Pupils and Students in all levels of education (ISCED 0-6) (share_studentsandpupils) , which is measured as % of total population at regional level .
The data is imported from the dataset EDUC_REGIND[^14].

[^14]: Source: <https://ec.europa.eu/eurostat/databrowser/view/educ_regind/default/table?lang=en&category=reg.reg_educ.reg_educ_97>[(europa.eu)](https://ec.europa.eu/eurostat/databrowser/view/educ_regind/default/table?lang=en&category=reg.reg_educ.reg_educ_97)
...
### New estimated model with 3 new variables

```{r}
#| echo: false
#| output: true
#| label: tbl-multiregression_with_3_new_variables
#| tbl-cap: Multiregression with 3 new variables

#| label: tbl-obs_2010
#| tbl-cap: Number of Observations by country in 2010

lm.3 <- lm(gini_n2 ~ log_GDP_pc , data = mlr2010)
...
The estimated model with all 4 variables

$$
y = -0.044 + 0.016*x_1 - 0.179*x_2 - 0.025*x_3 - 0.052*x_4 + u
$$We see that the intercept for this model is negative.
That is impossible because the gini coefficient can never be negative if all other variables are 0.

The only variable that increase the gini-coefficient by a one percent increase is log_GDP_pc.
The other variables decrease the gini_coefficient by an increase of one unit.
...
### Overall fit of new model

As before we use \$R_2\$ to measure goodness of fit.
The overall fit of my new model has an $R_2$ of 0.166, which means that the 4 variables explain 16.6 % of the variarion in the gini-coefficient.

We can see that the variable (of the new ones) that add most explanation to $R_2$ is Railway_km and least explanation is added by share_studentsand pupils.
If we include log_GDP_pc, this is the variable that adds least explanation to $R_2$

### Inference related to findings

In @tbl-multiregression_with_3_new_variables we can see that The only model that has an significant variable (of the three new) is Model 5, and the only variable that is significant is railway_km.
The variable is significant on a 10 % level, p \< 0.1.
This means that railway_km has an significant effect on the gini-coefficient.

The other variables in model 5 are insignificant on all levels.
This may indicate that it could be wise to just include Railway_km (of the new variables in the new estimated linear model. The linear model will then be either

$$
gini = 0.007 + 0.006x_1 - 0.01x_2
$$

or

$$
gini = - 0.044 + 0.016x_1-0.025x_2
$$

where $x_1$ is log_GDP_pc and $x_2$ is Railway_km.

# Mandatory Assignment 3

I have not been able to complete this assignment, due to migraine over many days.
...
[Cubic Functions in Econometrics Are Good for Inflexion - dummies](https://www.dummies.com/article/business-careers-money/business/economics/cubic-functions-in-econometrics-are-good-for-inflexion-156413/)

Explore alternative functional forms (e.g. log, cubical estimates, \...) of your previous estimates on the relationship between regional development yjyj on regional inequality GINIWjGINIWj .
Give an argument for two forms that might fit the data better.
...
-   Plot the corresponding scatter plots with the fitted estimate line.
    ...
    ```{r}
    #scatterplot for cubic function
    data2010_nuts2 %>% 
      ggplot(mapping = aes(x = GDP_per_capita, y = gini_n2)) +
      geom_point() +
...
-   Penal Estimates
    ...
    -   Use your entire data set to estimate the effect of regional development yjyj on regional inequality GINIWjGINIWj in a penal setup.
        Present estimates that contain the following fixed effects separately and jointly:
...
-   Clean up
    ...
    -   Your final submission must have less than 10.000 words, three tables and five figures.
        Appendix and references not included.
